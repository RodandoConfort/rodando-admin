...
import { VehicleUsageItem } from '../data-access/reports.models';
...
function vehicleLabel(item: VehicleUsageItem): string {
  return item.vehicleLabel?.trim() || item.vehicleId.slice(0, 8);
}

export function buildVehicleKmOptions(
...
export function buildVehicleRevenueOptions(
  items: VehicleUsageItem[],
): EChartsCoreOption {
  return {
    color: [colors.red],
...
export function buildVehicleTripsMinutesOptions(
  items: VehicleUsageItem[],
): EChartsCoreOption {
  return {
    ...
    yAxis: [
      ...
      },
      {
        ...
        axisLabel: {
          ...axisText(),
          formatter: (value: number) => `${compact(value)} min`,
        },
        splitLine: {
          show: false,
        },
    ...
    series: [
      ...
      },
      {
        ...
        itemStyle: {
          borderRadius: [10, 10, 0, 0],
          color: verticalGradient('#fcd34d', '#d97706'),
        },
      },
    ],
  };
...
export function buildVehicleRevenueShareOptions(
  items: VehicleUsageItem[],
): EChartsCoreOption {
  return {
    ...
    series: [
      {
        ...
        data: items
          .map((item) => ({
            ...
            value: Number(item.grossRevenue ?? 0),
          }))
